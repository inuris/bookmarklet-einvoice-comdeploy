...
async function processCom(taxCode){
	...
	if (comInfo && comInfo.comId && comInfo.comName) {
		...
		updateOverlayLog("Mã công ty: [" + comInfo.comId + "]", "Chuẩn bị khởi tạo <a href='https://quantridichvu-hddt.vnpt-invoice.com.vn/Initialization/CustomersReal?keyword=" + comInfo.taxCode + "&areaId=0&sysType=0&statustype=34&initType=0' style='color: #d00; text-decoration: underline;'>[Click để hủy]</a>");
	}
	else {
	...
	if (contract) {
		updateOverlayLog("Đã có hợp đồng. Bắt đầu khởi tạo thật...");
	}
	else {
		...
		if (qty && qty > 0) {
			...
			if (dataContract.success) {
				updateOverlayLog("Hoàn tất bổ sung hợp đồng. Bắt đầu khởi tạo thật...");
			}
			else {
				updateOverlayLog("<span style='color: #f00'>Không bổ sung được hợp đồng</span>");
				return false;
			}
		}
		else {
			updateOverlayLog("<span style='color: #f00'>Nhập sai số lượng hóa đơn</span>");
			return false;
		}
	}
	await wait(2000);
	...
	if (responseDeploy) {
		...
		updateOverlayLog("Lỗi khi Khởi tạo...");
		return false;
	}
	await wait(2000);
	...
	if (responseDomain) {
		...
		updateOverlayLog("Lỗi khi Tạo domain...");
		return false;
	}
}
